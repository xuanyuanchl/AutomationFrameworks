...
class LowLevelKeyword():
    ...
    def _deco(self, func, *args):
        ...
        try:
            ...
            self.testStepResult.stepLog = str(self.testStepResult.endTime) + ' ' + func.__name__ + ' execution was ' + StepResult.Error.name + '. Actual exception : ' + str(e)
        else :
            webdriverFractory.waitforready(self.settings.wedriver)
            self.testStepResult.stepResult = StepResult.Passed
            self.testStepResult.endTime = Tool.CurrentTime()
            self.testStepResult.stepLog = str(self.testStepResult.endTime) + ' ' + func.__name__ + ' execution was ' + self.testStepResult.stepResult.name
        finally:
    ...
    def select(self, elementExpression, value):
        __webelement : Select = self.findobject(elementExpression)
    ...
    def checkElementEnabled(self, elementExpression):
        __webelement :WebElement = self.findobject(elementExpression)
    ...
    def getCSSValue(self, element, cssPropertyName):
        __webelement :WebElement = self.findobject(element)
    ...
    def switchToWindow(self, index: int):
        self.settings.wedriver.switch_to.window(self.settings.wedriver.window_handles[index])

    def switchToFrame(self, frameId):
        self.settings.wedriver.switch_to.frame(self.settings.wedriver.find_element_by_xpath('//*[@id="{0}"]'.format(frameId)))
    ...
    def findobject(self, elementExpression:dict):
        ...
        __webelement :WebElement = self.settings.wedriver.find_element(by, objectstring)
        self.settings.wedriver.execute_script("arguments[0].style.border = \"5px solid yellow\"", __webelement)
        return __webelement