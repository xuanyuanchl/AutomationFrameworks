...
class Settings(object):
    ...
    def constructSettings():
        ...
        Settings.CurrentTestResultFolder = currentWD + '\\TestResult\\' + folderName + '\\' + Settings.planFile
        os.makedirs(Settings.CurrentTestResultFolder)
        Settings.testPlanResult = TestPlanResult("TestPlan")
        LOG_FORMAT = "%(asctime)s - %(levelname)s - %(message)s"
        logging.basicConfig(filename = os.path.join(currentWD, 'TestResult',folderName,'runtime_{0}.log'.format(folderName)), level=logging.INFO, format=LOG_FORMAT)
    ...
    def createWebDriver(self):
        self.wdf = webdriverFractory()
        self.wedriver = self.wdf.create()
    ...
    def loadPanFileAndSetCaseNumber(self):
        ...
        logging.info('open plan file: {0} successfully'.format(self.planFilePath))
    ...
    def loadScriptFile(self):
        ...
        logging.info('open script file: "{0}" successfully'.format(self.CurrentScriptPath))
        return wb_state
    ...
    def setStateFile(self, stateSheet:Worksheet, rowNum:int):
        ...
        self.StateFilePath = self.CurrentScriptPath.split('.xlsx')[0] + '/' + self.StateFile
    
    def updateStateResult(self, stateSheet:Worksheet, testStateResult, rowNum:int):
        testStateResult.Attachment = self.StateFile
        testStateResult.TestStateDescription = stateSheet.cell(rowNum,1).value if stateSheet.cell(rowNum,1).value != None else ''
    ...
    def updateScriptFileAndCaseResult(self, workB:Workbook, testCaseResult:TestCaseResult, caseResult):
        workB.save(self.CurrentScriptPath)
        logging.info('save scenario file: "{0}" successfully'.format(self.CurrentScriptPath))
    ...
    def updatePlanFile(self, planSheet:Worksheet, rowNum):
        planSheet.cell(rowNum,3).value = self.testPlanResult.TestCaseResultCollection[rowNum - 2].StartTime
        planSheet.cell(rowNum,4).value = self.testPlanResult.TestCaseResultCollection[rowNum - 2].EndTime
        planSheet.cell(rowNum,5).value = self.testPlanResult.TestCaseResultCollection[rowNum - 2].CaseResult        
        ...
        for para in parameters:
            ...
            if(param.count == 1):
                ...
                if (param[1].find("!") == -1):
                    ...
                    paraValue = Settings.StoredParameters[param[1].split('!')[1].strip()]
