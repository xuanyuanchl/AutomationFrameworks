...
class MainFramework(unittest.TestCase):
    ...
    def testExecute(self):
        try:
            ...
            for i in range(2, self.settings.TestCaseNumbers + 2):
                self.testCaseResult = self.settings.cleanUpAndSetUpPlanFile(wk_plan_sheet, i)
                ...
                try:
                    ...
                    for i in range(2, testStateRow + 2):
                        ...
                        self.testStateResult = TestStateResult(self.settings.StateFile)
                        ef = ExecuteFunction()
                        self.testStateResult = ef.SetState(self.settings.StateFile)
                        self.testStateResult = self.settings.updateStateResult(wk_script_sheet, 
                            self.testStateResult, i)
                        self.testCaseResult.TestStateResultCollection.append(self.testStateResult)
            ...
            logging.info('save plan file: "{0}" successfully'.format(self.settings.planFilePath))
    ...
    def tearDownClass(cls):
        ...
        logging.info('create report "{0}" successfully'.format(systemReport.planInfoHtml))
